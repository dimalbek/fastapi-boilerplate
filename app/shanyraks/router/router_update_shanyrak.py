...
def update_shanyrak(
    ...
    inp = input.dict()
    inp["location"] = svc.here_service.get_coordinates(input.address)  

    # location = svc.here_service.get_coordinates(input.address)
    # loc = {"location": location}
    # inp = input.dict()
    # inp.update(loc)
    update_result = svc.repository.update_shanyrak_by_id(shanyrak_id=shanyrak_id, user_id=jwt_data.user_id, data=inp)
